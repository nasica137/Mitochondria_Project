...
# Load the JSON data
with open('finetune-config1.json', 'r') as json_file:
...
encoder_variations = ['resnet50', 'vgg16']
...
for encoder_name in encoder_variations:
    for loss_name in loss_variations:
        ...
        with open('temp_config3.json', 'w') as temp_file:
            json.dump(config, temp_file)

        # Execute the Python file with subprocess module
        subprocess.run(['python', 'train_not_pretrained.py', '--config', 'temp_config3.json'])
