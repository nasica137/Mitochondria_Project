...
# Load the JSON data
with open('finetune-config_multiclass.json', 'r') as json_file:
...
for encoder_name in encoder_variations:
    for loss_name in loss_variations:
        ...
        subprocess.run(['python', 'multiclass_predict_large_image.py', '--config', 'temp_config.json'])
        
        # Execute the Python file with subprocess module
        #subprocess.run(['python', 'predict_treated_data.py', '--config', 'temp_config.json'])
