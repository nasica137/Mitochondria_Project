...
parser = argparse.ArgumentParser(description='Perform training on a large image.')
...
os.makedirs('pretrained/' + output_directory, exist_ok=True)
...
plt.savefig(f"./pretrained/{output_directory}/six_samples.png", bbox_inches='tight', pad_inches=0.1)
...
for i in range(0, epochs):
    ...
    if best_iou_score < valid_logs['iou_score']:
        ...
        torch.save(model, f'./pretrained/{output_directory}/best_model.pth')
        print('Model saved!')

print(f"Finished training {encoder_name} with {loss_name}")
#-----------------------------------------------------------------------------------------------------------------------
# Plotting
#-----------------------------------------------------------------------------------------------------------------------
...
train_logs_df.to_csv(f'./pretrained/{output_directory}/train_logs.csv', index=False)
valid_logs_df.to_csv(f'./pretrained/{output_directory}/valid_logs.csv', index=False)
...
plt.grid()
plt.savefig(f'./pretrained/{output_directory}/iou_score_plot.png')
...
plt.legend(loc='best', fontsize=16)
plt.grid()
plt.savefig(f'./pretrained/{output_directory}/{loss_name}_plot.png')