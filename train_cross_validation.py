...
for fold, (train_idx, val_idx) in enumerate(skf.split(all_paths, all_labels)):
    ...
    #pretrained_model_path = './fine-tuning/Adam/DiceLoss/Unet_mobilenet_v2/Unet_mobilenet_v2_100epochs_bs4_lr_0.0001/best_model.pth'
    pretrained_model_path = './fine-tuning/Adam/DiceFocalLoss/Unet_vgg16/Unet_vgg16_100epochs_bs4_lr_0.0001/best_model.pth'
    ...
    for i in range(0, epochs):
        ...
        if best_iou_score < valid_logs['iou_score']:
            ...
            torch.save(model, f'./multi_class_cross_validation/{output_directory}/best_model_fold_{fold}.pth')
            print('Model saved!')
    ...
    train_logs_df.to_csv(f'./multi_class_cross_validation/{output_directory}/train_logs_fold_{fold}.csv', index=False)
    valid_logs_df.to_csv(f'./multi_class_cross_validation/{output_directory}/valid_logs_fold_{fold}.csv', index=False)
    ...
    plt.grid()
    plt.savefig(f'./multi_class_cross_validation/{output_directory}/iou_score_plot_fold_{fold}.png')
    ...
    plt.legend(loc='best', fontsize=16)
    plt.grid()
    plt.savefig(f'./multi_class_cross_validation/{output_directory}/{loss_name}_plot_fold_{fold}.png')
...
performance_df.to_csv(f'./multi_class_cross_validation/{output_directory}/performance_metrics.csv', index=False)

# Print or save the summary DataFrame
print(summary_df)
summary_df.to_csv(f'./multi_class_cross_validation/{output_directory}/avg_max_performance_metrics.csv', index=False)
