...
os.makedirs('multi_class_cross_validation_not_pretrained/' + output_directory, exist_ok=True)
...
treated_images = [os.path.join(images_dir, img) for img in os.listdir(images_dir) if img.endswith(".png") and img.startswith("embl")]
treated_masks = [os.path.join(masks_dir, mask) for mask in os.listdir(masks_dir) if mask.endswith(".png") and mask.startswith("embl")]

untreated_images = [os.path.join(images_dir, img) for img in os.listdir(images_dir) if img.endswith(".png") and not img.startswith("embl")]
untreated_masks = [os.path.join(masks_dir, mask) for mask in os.listdir(masks_dir) if mask.endswith(".png") and not mask.startswith("embl")]

treated_images.sort()
treated_masks.sort()

untreated_images.sort()
untreated_masks.sort()
...
# Initialize a dictionary to store performance metrics for each fold
performance_metrics = {
    "Fold": [],
    "Accuracy": [],
    "Precision": [],
...
# Assign labels (0 for untreated, 1 for treated)
labels = [0] * len(untreated_masks) + [1] * len(treated_masks)

# Combine data and labels
all_data = list(zip(untreated_images, untreated_masks, [0]*len(untreated_images))) + \
           list(zip(treated_images, treated_masks, [1]*len(treated_images)))
...
all_paths = all_data[:, 0]
all_masks = all_data[:, 1]
all_labels = all_data[:, 2]

# Initialize StratifiedKFold with 5 folds
skf = StratifiedKFold(n_splits=5, shuffle=True, random_state=42)

print(f"Starting multiclass 5-fold cross_validation {encoder_name} with {loss_name}")
...
for fold, (train_idx, val_idx) in enumerate(skf.split(all_paths, all_labels)):
    ...
    train_transforms = Compose([
        ...
        Rand2DElasticd(keys=['image', 'mask'], prob=1.0, spacing=20, magnitude_range=(0,1)),
    ...
    # Define your validation and training files
    train_files = [{"image": img, "mask": mask} for img, mask in zip(fold_train_paths, fold_train_masks)]
    val_files = [{"image": img, "mask": mask} for img, mask in zip(fold_val_paths, fold_val_masks)]
    ...
    plt.savefig(f"./multi_class_cross_validation_not_pretrained/{output_directory}/six_samples.png", bbox_inches='tight', pad_inches=0.1)
    ...
    #pretrained_model_path = './not_pretrained/Adam/DiceFocalLoss/Unet_vgg16/Unet_vgg16_1000epochs_bs4_lr_0.0001/best_model.pth'
    pretrained_model_path = './not_pretrained/Adam/DiceLoss/Unet_mobilenet_v2/Unet_mobilenet_v2_1000epochs_bs4_lr_0.0001/best_model.pth'
    ...
    # Define per-class IoU metric
    class IoU(smp.utils.metrics.IoU):
        def __init__(self, threshold=0.5, per_class=True):
            super().__init__(threshold=threshold)
            self.per_class = per_class

        def forward(self, y_pr, y_gt):
            iou_class = super().forward(y_pr, y_gt)
            if self.per_class:
                return iou_class.mean(dim=0)
            else:
                return iou_class.mean()
    ...
    metrics = [
        ...
        #IoU(threshold=0.5, per_class=True),  # Set per_class to True for per-class IoU
    ]
    ...
    for i in range(0, epochs):
        ...
        if best_iou_score < valid_logs['iou_score']:
            ...
            torch.save(model, f'./multi_class_cross_validation_not_pretrained/{output_directory}/best_model_fold_{fold}.pth')
            print('Model saved!')
    ...
    print(f"Finished multiclass cross_validation {encoder_name} with {loss_name}")
    #-----------------------------------------------------------------------------------------------------------------------
    # Plotting
    #-----------------------------------------------------------------------------------------------------------------------
    ...
    train_logs_df.to_csv(f'./multi_class_cross_validation_not_pretrained/{output_directory}/train_logs_fold_{fold}.csv', index=False)
    valid_logs_df.to_csv(f'./multi_class_cross_validation_not_pretrained/{output_directory}/valid_logs_fold_{fold}.csv', index=False)
    ...
    plt.grid()
    plt.savefig(f'./multi_class_cross_validation_not_pretrained/{output_directory}/iou_score_plot_fold_{fold}.png')
    ...
    plt.legend(loc='best', fontsize=16)
    plt.grid()
    plt.savefig(f'./multi_class_cross_validation_not_pretrained/{output_directory}/{loss_name}_plot_fold_{fold}.png')
    
    
    # Get validation performance metrics
    valid_logs_df = pd.DataFrame(valid_logs_list)
    # Get the index of the maximum IoU
    max_iou_index = valid_logs_df["iou_score"].idxmax()

    # Save the performance metrics for this fold
    performance_metrics["Fold"].append(fold + 1)
    performance_metrics["Accuracy"].append(valid_logs_df["accuracy"].iloc[max_iou_index])
    performance_metrics["Precision"].append(valid_logs_df["precision"].iloc[max_iou_index])
    performance_metrics["Recall"].append(valid_logs_df["recall"].iloc[max_iou_index])
    performance_metrics["IoU"].append(valid_logs_df["iou_score"].max())
...
# Calculate average and maximum values across all folds
average_values = performance_df.mean(numeric_only=True)
max_values = performance_df.max(numeric_only=True)

# Create a DataFrame for average and maximum values
summary_df = pd.DataFrame({'Metric': ['Average', 'Max'],
                            'Accuracy': [average_values['Accuracy'], max_values['Accuracy']],
                            'Precision': [average_values['Precision'], max_values['Precision']],
                            'Recall': [average_values['Recall'], max_values['Recall']],
                            'IoU': [average_values['IoU'], max_values['IoU']]})

# Print or save the performance DataFrame
print(performance_df)
performance_df.to_csv(f'./multi_class_cross_validation_not_pretrained/{output_directory}/performance_metrics.csv', index=False)

# Print or save the summary DataFrame
print(summary_df)
summary_df.to_csv(f'./multi_class_cross_validation_not_pretrained/{output_directory}/avg_max_performance_metrics.csv', index=False)
