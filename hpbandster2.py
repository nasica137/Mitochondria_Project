...
from monai.transforms import Compose, LoadImaged, EnsureChannelFirstd  # Import aller benötigten Transformationen
...
# Lade die Konfigurationsdatei über die Kommandozeile
...
finetuned_directory = output_directory
...
# Definiere Transformationen für jede Art von Augmentierung
...
final_transforms = Compose([
                ...
                ConvertToMultiChannelMask(keys=["mask"])])

val_transforms = Compose(baseline_transforms.transforms + final_transforms.transforms)



blur_variants = [(0.25, 1.5),  (0.125, 0.75), (0.5, 3)]
elastic_variants = [(0, 1), (0, 2), (1,3)]

augmentation_variants = ['baseline', 'geometric', 'pixel-level', 'combined', 'blur', 'elastic']

value = 'default'

# Define mappings for augmentation types to their respective values
augmentation_mapping = {
    'blur': blur_variants,
    'elastic': elastic_variants
}


# Für jede Augmentierungsvariante und jede Kombination von Parametern führe das Training durch
for augmentation in augmentation_variants:
    # Check if augmentation is 'blur' or 'elastic'
    if augmentation in augmentation_mapping:
        augmentation_values = augmentation_mapping[augmentation]
        
        for value in augmentation_values:
            
            output_directory = f"{encoder_name}_{loss_name}_{augmentation}_{value}/"
            # Create the directory if it doesn't exist
            os.makedirs('multi_class_hpbandster_not_pretrained/' + output_directory, exist_ok=True)
            ...
            geometric_transforms = Compose([
                RandFlipd(keys=["image", "mask"], prob=1.0),
                RandRotate90d(keys=["image", "mask"], prob=1.0, spatial_axes=[0, 1]),
                RandZoomd(keys=["image", "mask"], prob=1.0, min_zoom=0.9, max_zoom=1.1, mode="nearest", padding_mode='constant'),
            ])
            

            print(augmentation)
            # Setze die Parameter für Blur, Elastic Deformation, Rotation, Zoom und Kontrast für das aktuelle Training
            if 'baseline' == augmentation:
                train_transforms = Compose(baseline_transforms.transforms + final_transforms.transforms)
                
            elif 'geometric' == augmentation:

                train_transforms = Compose(baseline_transforms.transforms + geometric_transforms.transforms + final_transforms.transforms)

            elif 'pixel-level' == augmentation:
                train_transforms = Compose(baseline_transforms.transforms + pixel_level_transforms.transforms + final_transforms.transforms)
                
            elif 'combined' == augmentation:
                train_transforms = Compose(baseline_transforms.transforms + geometric_transforms.transforms + pixel_level_transforms.transforms + final_transforms.transforms)
            elif 'blur' == augmentation:
                blur_transforms = Compose([
                    RandGaussianSmoothd(keys=["image"], prob=1.0, sigma_x=value, sigma_y=value)
                ])
            
                train_transforms = Compose(baseline_transforms.transforms + blur_transforms.transforms + final_transforms.transforms)
            elif 'elastic' == augmentation:
                elastic_transforms = Compose([
                    Rand2DElasticd(keys=['image', 'mask'], prob=1.0, spacing=20, magnitude_range=value)
                ])
                train_transforms = Compose(baseline_transforms.transforms + elastic_transforms.transforms + final_transforms.transforms)
            ...
            plt.savefig(f"./multi_class_hpbandster/{output_directory}/six_samples.png", bbox_inches='tight', pad_inches=0.1)
            ...
            pretrained_model_path = f'./not_pretrained/{finetuned_directory}/best_model.pth'
            ...
            for i in range(0, epochs):
                ...
                if best_iou_score < valid_logs['iou_score']:
                    ...
                    torch.save(model, f'./multi_class_hpbandster_not_pretrained/{output_directory}/best_model.pth')
                    print('Model saved!')
            ...
            print(f"Finished multiclass not_pretrained {encoder_name} with {loss_name}")
            #-----------------------------------------------------------------------------------------------------------------------
            # Plotting
            #-----------------------------------------------------------------------------------------------------------------------
            ...
            train_logs_df.to_csv(f'./multi_class_hpbandster_not_pretrained/{output_directory}/train_logs.csv', index=False)
            valid_logs_df.to_csv(f'./multi_class_hpbandster_not_pretrained/{output_directory}/valid_logs.csv', index=False)
            ...
            plt.grid()
            plt.savefig(f'./multi_class_hpbandster_not_pretrained/{output_directory}/iou_score_plot.png')
            ...
            plt.legend(loc='best', fontsize=16)
            plt.grid()
            plt.savefig(f'./multi_class_hpbandster_not_pretrained/{output_directory}/{loss_name}_plot.png')
    
    else:
        # augmentation is not blur/elastic
        value = 'default'
        output_directory = f"{encoder_name}_{loss_name}_{augmentation}_{value}/"
        # Create the directory if it doesn't exist
        os.makedirs('multi_class_hpbandster_not_pretrained/' + output_directory, exist_ok=True)
        ...
        plt.savefig(f"./multi_class_hpbandster_not_pretrained/{output_directory}/six_samples.png", bbox_inches='tight', pad_inches=0.1)
