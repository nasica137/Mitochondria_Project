...
train_transforms = Compose([
    ...
    Rand2DElasticd(keys=["image", "mask"], prob=0.7, spacing=20, magnitude_range=(1, 3)),
    RandGaussianNoised(keys=["image"], prob=0.5, mean=0.0, std=0.1),
    RandGaussianSmoothd(keys=["image", "mask"], prob=0.5),  
    RandAdjustContrastd(keys=["image", "mask"], prob=0.5, gamma=(0.5, 1.5)),
...
plt.savefig(f"./not_pretrained/{output_directory}/six_samples.png", bbox_inches='tight', pad_inches=0.1)
...
# Find the model path starting with "encoder_name"
local_model_path = glob(os.path.join("/misc/lmbraid21/nasica/pytorch/models", f"{encoder_name}*.pth"))[0]
...
for i in range(0, epochs):
    ...
    if best_iou_score < valid_logs['iou_score']:
        ...
        torch.save(model, f'./not_pretrained/{output_directory}/best_model.pth')
        print('Model saved!')

print(f"Finished training not_pretrained {encoder_name} with {loss_name}")
#-----------------------------------------------------------------------------------------------------------------------
# Plotting
#-----------------------------------------------------------------------------------------------------------------------
...
train_logs_df.to_csv(f'./not_pretrained/{output_directory}/train_logs.csv', index=False)
valid_logs_df.to_csv(f'./not_pretrained/{output_directory}/valid_logs.csv', index=False)
...
plt.grid()
plt.savefig(f'./not_pretrained/{output_directory}/iou_score_plot.png')
...
plt.legend(loc='best', fontsize=16)
plt.grid()
plt.savefig(f'./not_pretrained/{output_directory}/{loss_name}_plot.png')